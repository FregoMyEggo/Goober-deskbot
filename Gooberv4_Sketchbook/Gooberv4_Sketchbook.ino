...
static uint32_t gLastOldAgeRollYear = 8;
...
static uint32_t GOOBER_YEAR_MS = 76400000UL;
static const float FEED_DECAY_ABOVE_HALF_PER_HOUR = 13.0f;
static const float FEED_DECAY_BELOW_HALF_PER_HOUR = 1.0f;
static const float HAPPINESS_DECAY_ABOVE_HALF_PER_HOUR = 6.0f;
static const float HAPPINESS_DECAY_BELOW_HALF_PER_HOUR = 2.0f;
static uint32_t gLastPetTickMs = 0;
static uint32_t gAgeAccumMs = 0;
static const int EVOLVE_B_MIN_AGE = 2;      // first yearly 1-in-3 roll for a -> b
static const int EVOLVE_C_MIN_AGE = 6;      // first yearly 1-in-3 roll for b -> c
static const int OLD_AGE_ROLL_START_AGE = 9; // first yearly 1-in-5 death roll
static const uint8_t EVOLVE_CHANCE_DENOM = 3;
...
static void updateHomeBlobAnimation() {
  ...
  uint32_t now = millis();
  bool wantBored = (gHappinessMeter < 55.0f);
...
static void animTimerCb(lv_timer_t* t) {
  ...
  if (currentScreen == SCREEN_INTRO && introPhase == INTRO_INCUBATING) {
    if (millis() - introStartMs >= 30000UL) {
...
static void updatePetTimers() {
  ...
  while (gAgeAccumMs >= GOOBER_YEAR_MS) {
    ...
    if (gGooberAgeYears >= OLD_AGE_ROLL_START_AGE && gGooberAgeYears > (int)gLastOldAgeRollYear) {
...
static void buildFeedScreen() {
  ...
  lv_label_set_text(hint, "pick a snack • swipe right when done");
